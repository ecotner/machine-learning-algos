...
To do:
    1) Add an early stopping detector - if the (average?) validation error is larger than some constant (maybe two?) times the minimum validation error, stop the training.
...
class ConvolutionalAutoencoder(object):
    ...
    # Training function
    def train(self, X_train, lr, max_epochs, batch_size, reg_lambda=0, X_val=None, reload_parameters=False, save_path=None, plot_every_n_steps=25, save_every_n_epochs=2):
        '''
...
class QNetwork(object):
    ...
    def define_graph(self, input_spec, conv_spec, dense_spec, activation='relu', regularization=None):
        ...
        with G.as_default():
            ...
            Q = tf.placeholder(dtype=tf.float32, shape=[c_out])
            Loss = tf.add(tf.reduce_mean((Y-Q)**2), reg_loss, name='Loss')
